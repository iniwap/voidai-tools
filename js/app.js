...
const { Icon, ToastProvider } = window.SharedComponents;
...
const App = () => {
    ...
    const renderContent = () => {
        if (view === 'home') {
            return (
                <div className="max-w-6xl mx-auto w-full py-12 px-4 animate-fade-in overflow-y-auto custom-scrollbar h-full">
                    <div className="text-center mb-12 mt-10">
                        <h1 className="text-5xl font-extrabold text-white mb-4 tracking-tight">Void<span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-pink-400">AI</span> Tools</h1>
                        <p className="text-slate-400">墨智工坊打造的一站式 AI 生产力工具集</p>
                    </div>
                    <div className="grid md:grid-cols-3 gap-6 pb-20">
                        {TOOLS_CONFIG.map(tool => (
                            <div key={tool.id} onClick={() => setView(tool.id)} className="glass-panel rounded-2xl cursor-pointer group relative overflow-hidden h-64 hover:border-indigo-500/50 transition-all duration-300">
                                <img src={tool.image} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700" onError={(e) => { e.target.style.display = 'none'; }} />
        ...
        const ToolComponent = window.Tools && window.Tools[TOOLS_CONFIG.find(t => t.id === view)?.component];
        return ToolComponent ? <div className="flex-1 w-full h-full p-4 md:p-6 overflow-hidden"><ToolComponent /></div> : <div>Loading...</div>;
    };
