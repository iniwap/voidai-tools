...
const MeMakerTool = () => {
    ...
    const copyText = (text) => { navigator.clipboard.writeText(text); toast("咒语已复制"); };

    // --- 切图逻辑 ---
    useEffect(() => {
        if (!cutterRaw) return;
        // 智能计算：不强制正方形，保持原图网格比例
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in">
            ...
            <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
                ...
                {/* === Mode 1: Prompt (独立布局，不显示预览) === */}
                {mode === 'prompt' && (
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto pb-10">
                            {/* Card 1 */}
                            <div className="glass-panel p-6 rounded-2xl relative group">
                                ...
                                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                                    <pre className="text-sm text-slate-300 font-mono whitespace-pre-wrap leading-relaxed select-all">{prompts.static}</pre>
                                </div>
                            </div>
                            {/* Card 2 */}
                            <div className="glass-panel p-6 rounded-2xl relative group">
                                ...
                                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                                    <pre className="text-sm text-slate-300 font-mono whitespace-pre-wrap leading-relaxed select-all">{prompts.anim}</pre>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-6 h-full">
                        ...
                        <div className="w-1/2 glass-panel p-6 rounded-2xl flex flex-col">
                            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2 border-b border-white/5 pb-3">
                                <Icon name="settings-2" className="text-indigo-400" /> 参数配置
                            </h3>
                            ...
                            <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2">
                                ...
                                <label className="block w-full aspect-[2/1] border-2 border-dashed border-slate-700 hover:border-indigo-500 hover:bg-slate-800/30 transition-all rounded-xl flex flex-col items-center justify-center cursor-pointer group relative overflow-hidden bg-slate-900/50">
                                    {(mode === 'cutter' ? cutterRaw : animRaw) ? (
                                        <>
                                            <img src={(mode === 'cutter' ? cutterRaw : animRaw).src} className="absolute inset-0 w-full h-full object-contain p-4 opacity-40 group-hover:opacity-20 transition-opacity" />
                                            <div className="relative z-10 flex flex-col items-center">
                                                <Icon name="refresh-ccw" size={32} className="text-white mb-2 shadow-sm" />
                                                <span className="text-sm font-bold text-white shadow-sm">点击更换图片</span>
                                            </div>
                                        </>
                                    ) : (
                                        <>
                                            <Icon name="upload-cloud" size={48} className="text-slate-600 mb-3 group-hover:text-indigo-400 transition-colors" />
                                            <span className="text-sm font-medium text-slate-400">点击上传 {mode === 'cutter' ? '网格图' : '序列图'}</span>
                                        </>
                                    )}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <label className="text-xs text-slate-400 font-bold uppercase">列数 (Cols)</label>
                                        <input type="number" value={mode === 'cutter' ? cutCols : animCols} onChange={e => mode === 'cutter' ? setCutCols(e.target.value) : setAnimCols(e.target.value)} className="w-full h-10 bg-slate-950 border border-slate-800 rounded-lg text-center text-white focus:border-indigo-500 outline-none" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-xs text-slate-400 font-bold uppercase">行数 (Rows)</label>
                                        <input type="number" value={mode === 'cutter' ? cutRows : animRows} onChange={e => mode === 'cutter' ? setCutRows(e.target.value) : setAnimRows(e.target.value)} className="w-full h-10 bg-slate-950 border border-slate-800 rounded-lg text-center text-white focus:border-indigo-500 outline-none" />
                                    </div>
                                </div>

                                {mode === 'animator' && (
                                    <div className="space-y-2">
                                        <div className="flex justify-between"><label className="text-xs text-slate-400 font-bold uppercase">帧率 (FPS)</label><span className="text-xs text-indigo-400 font-mono">{fps}</span></div>
                                        <input type="range" min="1" max="24" value={fps} onChange={e => setFps(e.target.value)} className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="pt-6 mt-auto border-t border-white/5">
                                {mode === 'cutter' ? (
                                    <Button variant="primary" className="w-full h-12 text-base" icon="package" disabled={!slices.length} onClick={downloadZip}>打包下载 ZIP</Button>
                                ) : (
                        ...
                        <div className="w-1/2 glass-panel p-6 rounded-2xl flex flex-col relative bg-slate-900/40">
                            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2 border-b border-white/5 pb-3">
                                <Icon name="eye" className="text-pink-400" /> 结果预览
                            </h3>
                                ...
                                {mode === 'animator' && (
                                    animRes ? (
                                        <div className="flex flex-col items-center gap-6 animate-fade-in">
                                            <img src={animRes} className="max-w-[80%] max-h-[300px] object-contain bg-checkerboard rounded-lg shadow-2xl border border-slate-700 p-2" />
